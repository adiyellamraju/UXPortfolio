...
E. Experimentation Infrastructure UX — Salesforce (2024). IMPORTANT TIMELINE NOTE: this shipped BEFORE the Personalization Core redesign, so its visual language is the older Salesforce chrome and it is NOT part of the redesigned Personalization Core. Don't describe it as a companion to that work.
  HOW HE GOT STAFFED: the Senior Director of Product asked Aditya's manager for design help on experimentation. He was staffed onto it as design lead. Core partnership team: a Director PM, a Senior PM, an engineering architect, and an engineering manager.
  THE PROBLEM: Salesforce customers wanted visibility and transparency into how decisions get configured and how they perform. They had no reliable way to establish the credibility of the content being delivered to their own customers, a gap that widened as AI and ML took over more of the decisioning. The engine was making more calls, more often, for more brands, but nothing let a marketer prove a call was the right one. Framing: "personalization without verification is just a guess at scale." The HMW: how might we enhance Customer 360 with experimentation capabilities that build trust and confidence in decisioning, anywhere in the platform, and move toward 1:1 hyper-personalization?
  AUDIENCE: marketing specialists and website admins. Neither is a statistician; both are accountable for what a customer sees. Two research quotes drove it: "When I'm configuring my personalization point, I want to experiment with multiple variations of decisions so I can trust and improve my recommendations," and "When I'm configuring my email journey, I want to randomly select audience members to experiment with multiple journey variations of the split so I can put people on the right journey path."
  PROCESS: he ran a workshop to unpack the problem (jobs-to-be-done mapping, clustered into four domains — Learn, Plan, Build, Run — which became the spine of the work), plus competitive teardowns of web and email experimentation tools (Optimizely, Adobe). Then he sat with the engineering architect and deconstructed the decisioning pipeline, annotating it with the plain-language questions design needed answered, and rewrote it as eight sentences the whole team could hold in their head: a user acts → match to a Data Cloud profile → qualify → is there an experiment (and is it an AI experiment) → slot into a cohort → run the experiment (A/B or bandit) → make the decision → publish and record. Translating architecture into plain language did more for cross-functional alignment than any wireframe.
  THE BIG CONFLICT (his most important story here — a disagree-and-commit masterclass): Personalization has two levels. A "personalization point" is a LOCATION (homepage hero, product-page slot). A "decision" is WHAT to show there and to whom. One personalization point can hold several decisions. Aditya argued the experiment should attach to the DECISION, because that's where the marketer has already declared an audience and content; attaching it a level up means re-asking for both. He built a mental model artifact and argued from it: "I have already defined my audience in a decision. Why should I redefine it for an experiment?" He got heavy pushback from the engineering architect because of how the system was already architected. This was engineering-first versus customer-first. HE LOST. Experimentation attached to the personalization point, which kept it platform-centric, easier to expose to other clouds, and made "experimentation as a service" possible. He committed and designed the best version of a model he'd argued against — which is why targeting rules and content reappear inside the wizard even though the decision already holds them.
  THE DESIGNS: a four-step wizard — Name → Metrics → Targeting → Cohorts — one decision per step, with a PERSISTENT explainer rail on the right (not a tooltip, not a separate guidance tab; experimentation is a vocabulary problem as much as a configuration problem). Key decisions: (1) inherited properties (data space, profile data graph, type, source, schema) shown as read-only chips instead of five more fields; (2) exactly ONE required primary metric, secondaries optional as removable pills — the constraint IS the design, because most tools let you pick a pile of metrics and sort out significance later, which is how a test ends with four numbers and no decision; (3) targeting reuses the same Resource/Operator/Value row as the rest of Personalization, resolving against live Data Cloud attributes so a rule can't be written the engine can't match; (4) a cohort is a percentage plus a piece of content and nothing else — control cohort is undeletable, and "No Recommender" is a first-class option because the most honest test of a recommender is often against no recommender; (5) the same cohort shell serves an ML recommender or hand-authored content, so one model covers "test two algorithms" and "test two headlines"; (6) finished cohorts collapse to summary rows that double as the audit trail, and a "Distribute Remaining Cohort Percentage" action does the allocation arithmetic rather than validating it afterward; (7) incomplete states point at themselves — a cohort row flagged "Define Content" instead of a disabled Next button.
  READING THE RESULT: he also designed the experiment summary/analytics surface. Statistics phrased as decisions, not hypotheses: "chance to beat control" with credible intervals instead of p-values; chance-to-beat-control AND chance-to-beat-all shown side by side (a variation can beat baseline and still lose to a sibling); the primary metric owns the chart with secondaries tabbed below; every result row names its content ("Variation 1 · Top Sellers") so an outcome traces back to the recommender that produced it; lift always signed and colored including losses, because a test that shows the AI losing is what makes the wins believable; and Stop Experiment as a primary action, since the fastest way to make people comfortable shipping an experiment is to make ending one obvious.
  OUTCOMES: the first experimentation service at Salesforce built to work two ways at once — natively inside Personalization AND as a service other clouds could call, i.e. experimentation as a service for Marketing Cloud and beyond. He validated the flow with 10 solution engineers and account executives; all were confident their accounts would use it and read it as an adoption driver for personalization overall. Next on the roadmap was AI in the loop with automated winner selection. THE REVERSAL: during the Personalization Core redesign, the head of engineering revisited the attachment question with customer feedback, research, and Aditya's mental model in hand, and came around. Experimentation 2.0 moved to the DECISION level. Aditya's last work before leaving Salesforce was redesigning it that way and handing it to another designer. The lesson: committing to a decision he disagreed with kept the business moving, and keeping the argument on the record meant it was still there when the evidence caught up.
